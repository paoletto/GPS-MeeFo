...
Page {
    ...
    function gpsWorker() {
        ...
        if (positionSource.position.longitudeValid == true && positionSource.position.latitudeValid == true && positionSource.position.altitudeValid == true) {
            ...
            if(samplenumber===1) {
                // FixMe: Support a proper QtLocation Map
...
//                    text: "Nokia Map of Average location"
//                    font.pixelSize: UI.FONT_DEFAULT
//                    color: !theme.inverted ? UI.COLOR_FOREGROUND : UI.COLOR_INVERTED_FOREGROUND //"#818181"
//                }

//                // Map via Web View TODO: Replace with a Map {} element
//                WebView {
//                    id: map3
//                    anchors.horizontalCenter: parent.horizontalCenter
...
//            }
    ...
    Menu {
        ...
        MenuItem {
            ...
            onTriggered: (units == "m") ? units = "f" : units = "m"
        }
        MenuItem {
            id: chooseNotation
            text: (unitsD == "g") ? "Use Decimal notation." : "Use Degrees notation."
            onTriggered: (unitsD == "g") ? unitsD = "d" : unitsD = "g"
        }
        MenuItem {
            id: chooseTheme
            text: (theme.inverted) ? "Use Light theme." : "Use Dark theme."
            onTriggered: (theme.inverted) ? (theme.inverted = false) : (theme.inverted = true)
        }
    }

    onVisibleChanged: {
        appWindow.toolBar = mainTools
    }
    ...
    ToolBar {
        ...
        RowLayout {
            ToolButton {
                icon.name: "toolbar-back"
                onClicked: Qt.quit()
            }
            ToolButton {
                icon.name: "toolbar-refresh"
                onClicked: {
                    positionSource.active = true
                    samplenumber = 0
                    avglat = 0

...
QList<QObject*> MySatsClass::getViewData() const
{
    ...
    foreach( QGeoSatelliteInfo x, satellitesInView){
        ...
        dataList.append( new ListItem( x.satelliteIdentifier(),
                                       x.signalStrength(),
                                       x.attribute(QGeoSatelliteInfo::Elevation),
                                       x.attribute(QGeoSatelliteInfo::Azimuth),
                                       rectx,
                                       recty  ) );
    }
    return dataList;
...
QList<QObject*> MySatsClass::getUseData() const
{
    ...
    foreach( QGeoSatelliteInfo x, satellitesInUse){
        ...
        dataListUse.append( new ListItem( x.satelliteIdentifier(),
                                          x.signalStrength(),
                                          x.attribute(QGeoSatelliteInfo::Elevation),
                                          x.attribute(QGeoSatelliteInfo::Azimuth),
                                          rectxUse, rectyUse  ) );
